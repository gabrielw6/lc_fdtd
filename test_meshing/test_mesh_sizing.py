...
from cavity_perturbation.meshing.mesh_sizing import characteristic_length, characteristic_length_from_cavity
...
def test_cavity_sourced_path_matches_explicit_path():
    a, b, c = 0.03, 0.04, 0.05
    cav = RectangularCavity(a, b, c, ModeIndex("TE", (0, 1, 1)))
    h_from_cavity = characteristic_length_from_cavity(cav, elements_per_wavelength=10)
    h_explicit = characteristic_length(cav.f0, cav.epsilon_bg, cav.mu_bg, elements_per_wavelength=10)
    assert h_from_cavity == pytest.approx(h_explicit, rel=1e-12)
