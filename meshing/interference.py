...
property query -- never `SampleRegion.contains()`/`.volume()`. Using
Module 3's own methods to validate geometry Module 3 also produced would
not be an independent check (Section 0.3/6.1) -- it would just test that a
copy of the same formula agrees with itself.
"""
