...
def _resolve_cavity_solid(cavity_input: CavityGeometryInput) -> DimTag:
    if isinstance(cavity_input, StandardCavityInput):
        return standard_shapes.build_cavity_solid(cavity_input.cavity_mode)
...
def _resolve_sample_solid(sample_input: SampleGeometryInput) -> DimTag:
    if isinstance(sample_input, StandardSampleInput):
        return standard_shapes.build_sample_solid(sample_input.region)
    ...
    # Section 0.4: a STEP sample's position relative to the cavity is an
    # explicit, required rigid transform -- applied before anything else
    # touches the shape.
    apply_to_shape(sample_input.transform, dim_tags)
    return dim_tags[0]


def _resolve_characteristic_length(
    cavity_input: CavityGeometryInput,
    target_elements_per_wavelength: int,
    reference_frequency: float | None,
    background_eps: complex | None,
    background_mu: complex | None,
) -> float:
    if isinstance(cavity_input, StandardCavityInput):
        # Section 0.6: f, eps_bg, mu_bg read directly off the CavityMode.
        return mesh_sizing.characteristic_length_from_cavity(
            cavity_input.cavity_mode, target_elements_per_wavelength
        )
    if reference_frequency is None:
        raise ValueError(
            "reference_frequency is required to size the mesh for a StepCavityInput "
            "(Section 0.6) -- there is no CavityMode to read f0/eps/mu from"
        )
    ...
    return mesh_sizing.characteristic_length(reference_frequency, eps, mu, target_elements_per_wavelength)
...
def build_mesh(
    ...
    `reference_frequency`/`background_eps`/`background_mu` are only used
    (and `reference_frequency` is only required) for a `StepCavityInput`,
    which has no `CavityMode` to read `f0`/`eps`/`mu` from (Section 0.6).
    """
    ...
    def compute() -> mesh_io.MeshResult:
        ...
        gmsh.model.add("cavity_perturbation_mesh")
        try:
            ...
            h_char = _resolve_characteristic_length(
                cavity_input, target_elements_per_wavelength, reference_frequency, background_eps, background_mu
            )
