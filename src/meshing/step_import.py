...
def import_step(path: Path | str, length_unit: str | None) -> list[DimTag]:
    ...
    always in SI units (meters), regardless of what the file itself claims
    or what Gmsh would otherwise default to. Requires an active Gmsh model
    (caller's responsibility -- this function doesn't initialize/finalize
    Gmsh itself)."""
